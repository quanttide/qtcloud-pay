...
- 新增拒绝转赠人工退款留痕：`POST /transfers/{id}/refund` 对 rejected 转赠加回购买人余额、写入退款留痕流水并关联 `refund_transaction_id`，权限为 `transfer:refund`。
...
- 转赠对账扩展拒绝退款链路：rejected 未退款视为正常待处理；已关联退款时校验退款流水存在且账户、金额、类型一致。
...
### Added
- 代金券计价规则集校验支持追加奖励：`bonus_type` 承载首次完成、产出评定、群内互动，`amount_rule` 承载动态等额或 100/500 元评定制金额规则。

### Changed
- `qtclass-voucher-pricing` 测试夹具同步支付工程档案最新版本，发行渠道由 4 个扩展为 7 个；规则快照仍只用于录入与查询，不改变发券/结算执行路径。
